...
> **몰입시간 정의 caveat**: 여기서 몰입시간 = `focus_time`(= 체류 − 외출 − 공용 − 상담). 단 `focus_time`은 **등원인정 교시를 못 빼** 일부 부풀려져 있다(평균 +6%, 헤비유저 +1.69h). patrol 기반 보정으로 재검증 시 몰입↔순위는 −0.924 → −0.921로 거의 불변(결론 견고). 상세: [데이터 품질 노트](../DATA_QUALITY_focus_time.md).
