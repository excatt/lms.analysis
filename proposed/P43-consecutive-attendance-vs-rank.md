...
- ✅ **outcome 정의와 무관한 유일한 견고 효과**: 핵심 순위 동인들(02·26·P42)을 **FOCUS_TIME 빌보드**로 재검증하면 대부분 소멸/반전하는데, 연속등원만 **FOCUS 순위에서도 +0.25로 생존**한다. 즉 02·P42가 STUDY_TIME 구성(공용·상담·체류)에 기댄 반면, 연속등원은 메트릭 선택과 무관한 진짜 '꾸준함' 효과다. 상세: [방법론 노트](../METHODOLOGY_billboard_choice.md).
- **즉시 감지 가능한 이탈 선행지표**: `consecutive_attendance`는 출결 시스템에서 실시간 산출되는 값이라, **연속등원 끊김**은 별도 모델 없이도 잡아낼 수 있는 이탈/슬럼프 후보 신호다.
